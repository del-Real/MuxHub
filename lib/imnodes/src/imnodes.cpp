...
inline ImVec2 EvalCubicBezier(const float t, const ImVec2 &P0, const ImVec2 &P1,
...
ImVec2 GetClosestPointOnCubicBezier(const int num_segments, const ImVec2 &p,
    ...
    for (int i = 1; i <= num_segments; ++i) {
        ImVec2 p_current =
            EvalCubicBezier(t_step * i, cb.P0, cb.P1, cb.P2, cb.P3);
...
inline float GetDistanceToCubicBezier(const ImVec2 &pos,
                                      const CubicBezier &cubic_bezier,
                                      const int num_segments) {
    const ImVec2 point_on_curve =
        GetClosestPointOnCubicBezier(num_segments, pos, cubic_bezier);
...
inline CubicBezier GetCubicBezier(ImVec2 start, ImVec2 end,
                                  const ImNodesAttributeType start_type,
                                  const float line_segments_per_length) {
    IM_ASSERT((start_type == ImNodesAttributeType_Input) ||
              (start_type == ImNodesAttributeType_Output));
...
inline float EvalImplicitLineEq(const ImVec2 &p1, const ImVec2 &p2,
                                const ImVec2 &p) {
...
inline bool RectangleOverlapsLineSegment(const ImRect &rect, const ImVec2 &p1,
    ...
    const int corner_signs[4] = {
        ...
        Sign(EvalImplicitLineEq(p1, p2,
                                ImVec2(flip_rect.Max.x, flip_rect.Min.y))),
        Sign(EvalImplicitLineEq(p1, p2,
                                ImVec2(flip_rect.Min.x, flip_rect.Max.y))),
...
inline bool RectangleOverlapsBezier(const ImRect &rectangle,
                                    const CubicBezier &cubic_bezier) {
    ImVec2 current = EvalCubicBezier(0.f, cubic_bezier.P0, cubic_bezier.P1,
                                     cubic_bezier.P2, cubic_bezier.P3);
    const float dt = 1.0f / cubic_bezier.NumSegments;
    for (int s = 0; s < cubic_bezier.NumSegments; ++s) {
        ImVec2 next =
            EvalCubicBezier(static_cast<float>((s + 1) * dt), cubic_bezier.P0,
...
inline bool RectangleOverlapsLink(const ImRect &rectangle, const ImVec2 &start,
                                  const ImVec2 &end,
                                  const ImNodesAttributeType start_type) {
    ...
    if (rectangle.Overlaps(lrect)) {
        ...
        const CubicBezier cubic_bezier = GetCubicBezier(
            start, end, start_type, GImNodes->Style.LinkLineSegmentsPerLength);
...
inline ImVec2 ScreenSpaceToGridSpace(const ImNodesEditorContext &editor,
                                     const ImVec2 &v) {
    return v - GImNodes->CanvasOriginScreenSpace - editor.Panning;
}

inline ImRect ScreenSpaceToGridSpace(const ImNodesEditorContext &editor,
                                     const ImRect &r) {
    return ImRect(ScreenSpaceToGridSpace(editor, r.Min),
                  ScreenSpaceToGridSpace(editor, r.Max));
}

inline ImVec2 GridSpaceToScreenSpace(const ImNodesEditorContext &editor,
                                     const ImVec2 &v) {
    return v + GImNodes->CanvasOriginScreenSpace + editor.Panning;
}

inline ImVec2 GridSpaceToEditorSpace(const ImNodesEditorContext &editor,
                                     const ImVec2 &v) {
    return v + editor.Panning;
}

inline ImVec2 EditorSpaceToGridSpace(const ImNodesEditorContext &editor,
                                     const ImVec2 &v) {
    return v - editor.Panning;
}
...
inline ImVec2 MiniMapSpaceToGridSpace(const ImNodesEditorContext &editor,
                                      const ImVec2 &v) {
    return (v - editor.MiniMapContentScreenSpace.Min) / editor.MiniMapScaling +
           editor.GridContentBounds.Min;
}

inline ImVec2 ScreenSpaceToMiniMapSpace(const ImNodesEditorContext &editor,
                                        const ImVec2 &v) {
...
inline ImRect ScreenSpaceToMiniMapSpace(const ImNodesEditorContext &editor,
                                        const ImRect &r) {
    return ImRect(ScreenSpaceToMiniMapSpace(editor, r.Min),
...
void DrawListSwapSubmissionIndices(const int lhs_idx, const int rhs_idx) {
    ...
    ImDrawListSplitterSwapChannels(GImNodes->CanvasDrawList->_Splitter,
...
void DrawListSortChannelsByDepth(const ImVector<int> &node_idx_depth_order) {
    ...
    while (node_idx_depth_order[start_idx] ==
           GImNodes->NodeIdxSubmissionOrder[start_idx]) {
        if (--start_idx == 0) {
    ...
    for (int depth_idx = start_idx; depth_idx > 0; --depth_idx) {
        ...
        for (int j = submission_idx; j < depth_idx; ++j) {
            ...
            ImSwap(GImNodes->NodeIdxSubmissionOrder[j],
                   GImNodes->NodeIdxSubmissionOrder[j + 1]);
        }
    }
}
...
ImVec2 GetScreenSpacePinCoordinates(const ImRect &node_rect,
                                    const ImRect &attribute_rect,
                                    const ImNodesAttributeType type) {
    IM_ASSERT(type == ImNodesAttributeType_Input ||
              type == ImNodesAttributeType_Output);
...
ImVec2 GetScreenSpacePinCoordinates(const ImNodesEditorContext &editor,
                                    const ImPinData &pin) {
    const ImRect &parent_node_rect = editor.Nodes.Pool[pin.ParentNodeIdx].Rect;
    return GetScreenSpacePinCoordinates(parent_node_rect, pin.AttributeRect,
                                        pin.Type);
}
...
void BeginNodeSelection(ImNodesEditorContext &editor, const int node_idx) {
    ...
    editor.PrimaryNodeOffset = ref_origin + GImNodes->CanvasOriginScreenSpace +
                               editor.Panning - GImNodes->MousePos;
...
void BeginLinkDetach(ImNodesEditorContext &editor, const int link_idx,
...
void BeginLinkCreation(ImNodesEditorContext &editor,
    ...
    editor.ClickInteraction.LinkCreation.Type =
...
void BeginLinkInteraction(ImNodesEditorContext &editor, const int link_idx,
    ...
    if (modifier_pressed) {
        ...
        if (pin_idx.HasValue()) {
            const int hovered_pin_flags =
                editor.Pins.Pool[pin_idx.Value()].Flags;

            // Check the 'click and drag to detach' case.
            if (hovered_pin_flags &
...
void BeginCanvasInteraction(ImNodesEditorContext &editor) {
    const bool any_ui_element_hovered = GImNodes->HoveredNodeIdx.HasValue() ||
    ...
    for (int link_idx = 0; link_idx < editor.Links.Pool.size(); ++link_idx) {
        if (editor.Links.InUse[link_idx]) {
            ...
            const ImRect &node_start_rect =
                editor.Nodes.Pool[pin_start.ParentNodeIdx].Rect;
            const ImRect &node_end_rect =
                editor.Nodes.Pool[pin_end.ParentNodeIdx].Rect;
...
void TranslateSelectedNodes(ImNodesEditorContext &editor) {
    if (GImNodes->LeftMouseDragging) {
        // If we have grid snap enabled, don't start moving nodes until we've
        // moved the mouse slightly
        const bool shouldTranslate =
            (GImNodes->Style.Flags & ImNodesStyleFlags_GridSnapping)
                ? ImGui::GetIO().MouseDragMaxDistanceSqr[0] > 5.0
                : true;

        const ImVec2 origin = SnapOriginToGrid(
            GImNodes->MousePos - GImNodes->CanvasOriginScreenSpace -
...
ImOptionalIndex FindDuplicateLink(const ImNodesEditorContext &editor,
...
bool ShouldLinkSnapToPin(const ImNodesEditorContext &editor,
                         const ImPinData &start_pin, const int hovered_pin_idx,
    ...
    if (duplicate_link.HasValue() &&
        !(duplicate_link == GImNodes->SnapLinkIdx)) {
        return false;
    }
...
void ClickInteractionUpdate(ImNodesEditorContext &editor) {
    ...
    case ImNodesClickInteractionType_BoxSelection: {
        ...
        const ImU32 box_selector_color =
            GImNodes->Style.Colors[ImNodesCol_BoxSelector];
        const ImU32 box_selector_outline =
            GImNodes->Style.Colors[ImNodesCol_BoxSelectorOutline];
        GImNodes->CanvasDrawList->AddRectFilled(box_rect.Min, box_rect.Max,
                                                box_selector_color);
        GImNodes->CanvasDrawList->AddRect(box_rect.Min, box_rect.Max,
    ...
    case ImNodesClickInteractionType_LinkCreation: {
        ...
        if (snapping_pin_changed && GImNodes->SnapLinkIdx.HasValue()) {
            BeginLinkDetach(
                editor, GImNodes->SnapLinkIdx.Value(),
                editor.ClickInteraction.LinkCreation.EndPinIdx.Value());
        }

        const ImVec2 start_pos =
            GetScreenSpacePinCoordinates(editor, start_pin);
        ...
        const CubicBezier cubic_bezier =
            GetCubicBezier(start_pos, end_pos, start_pin.Type,
        ...
        GImNodes->CanvasDrawList->AddBezierCubic(
            ...
            GImNodes->Style.LinkThickness, cubic_bezier.NumSegments);
        ...
        if (create_link && !maybe_duplicate_link_idx.HasValue()) {
            // Avoid send OnLinkCreated() events every frame if the snap link is
            // not saved (only applies for EnableLinkCreationOnSnap)
            if (!GImNodes->LeftMouseReleased &&
                editor.ClickInteraction.LinkCreation.EndPinIdx ==
                    GImNodes->HoveredPinIdx) {
                break;
...
void ResolveOccludedPins(const ImNodesEditorContext &editor,
    ...
    for (int depth_idx = 0; depth_idx < (depth_stack.Size - 1); ++depth_idx) {
        const ImNodeData &node_below =
            editor.Nodes.Pool[depth_stack[depth_idx]];

        // Iterate over the rest of the depth stack to find nodes overlapping
        // the pins
        for (int next_depth_idx = depth_idx + 1;
             next_depth_idx < depth_stack.Size; ++next_depth_idx) {
            const ImRect &rect_above =
                editor.Nodes.Pool[depth_stack[next_depth_idx]].Rect;
...
ImOptionalIndex ResolveHoveredPin(const ImObjectPool<ImPinData> &pins,
    ...
    const float hover_radius_sqr =
        GImNodes->Style.PinHoverRadius * GImNodes->Style.PinHoverRadius;
    ...
    for (int idx = 0; idx < pins.Pool.Size; ++idx) {
        ...
        // TODO: GImNodes->Style.PinHoverRadius needs to be copied into pin data
        // and the pin-local value used here. This is no longer called in
        // BeginAttribute/EndAttribute scope and the detected pin might have a
        // different hover radius than what the user had when calling
        // BeginAttribute/EndAttribute.
        if (distance_sqr < hover_radius_sqr &&
...
ImOptionalIndex ResolveHoveredLink(const ImObjectPool<ImLinkData> &links,
    ...
    // 1. The link is within hover distance to the mouse. The closest such link
    // is selected as being hovered over.
    // 2. If the link is connected to the currently hovered pin.
    //
    // The latter is a requirement for link detaching with drag click to work,
    // as both a link and pin are required to be hovered over for the feature to
    // work.
    ...
    for (int idx = 0; idx < links.Pool.Size; ++idx) {
        ...
        // If there is a hovered pin links can only be considered hovered if
        // they use that pin
        ...
        // TODO: the calculated CubicBeziers could be cached since we generate
        // them again when rendering the links

        const CubicBezier cubic_bezier =
            GetCubicBezier(start_pin.Pos, end_pin.Pos, start_pin.Type,
                           GImNodes->Style.LinkLineSegmentsPerLength);

        // The distance test
        {
            const ImRect link_rect =
                GetContainingRectForCubicBezier(cubic_bezier);

            // First, do a simple bounding box test against the box containing
            // the link to see whether calculating the distance to the link is
            // worth doing.
            if (link_rect.Contains(GImNodes->MousePos)) {
                ...
                // TODO: GImNodes->Style.LinkHoverDistance could be also copied
                // into ImLinkData, since we're not calling this function in the
                // same scope as ImNodes::Link(). The rendered/detected link
                // might have a different hover distance than what the user had
                // specified when calling Link()
...
void DrawGrid(ImNodesEditorContext &editor, const ImVec2 &canvas_size) {
    ...
    bool draw_primary =
        GImNodes->Style.Flags & ImNodesStyleFlags_GridLinesPrimary;

    /*
    // Draw dotted grid
    float dot_spacing = 30.0f; // Space between dots, adjust as needed
    float dot_size = 1.0f;     // Size of each dot

    // Draw horizontal grid dots
    for (float x = fmodf(offset.x, GImNodes->Style.GridSpacing);
         x < canvas_size.x; x += GImNodes->Style.GridSpacing) {

        // Determine if this is a primary grid line
        ImU32 dot_color = (offset.x - x == 0.f && draw_primary) ?
    line_color_prim : line_color;

        // Draw dots along this vertical line
        for (float y = fmodf(offset.y, dot_spacing);
             y < canvas_size.y; y += dot_spacing) {

            GImNodes->CanvasDrawList->AddCircleFilled(
                EditorSpaceToScreenSpace(ImVec2(x, y)),
                dot_size,
                dot_color);
        }
    }

    // Draw vertical grid dots
    for (float y = fmodf(offset.y, GImNodes->Style.GridSpacing);
         y < canvas_size.y; y += GImNodes->Style.GridSpacing) {

        // Determine if this is a primary grid line
        ImU32 dot_color = (offset.y - y == 0.f && draw_primary) ?
    line_color_prim : line_color;

        // Skip vertical lines that are already covered by horizontal grid
        if (fmodf(y, GImNodes->Style.GridSpacing) == 0.0f)
            continue;

        // Draw dots along this horizontal line
        for (float x = fmodf(offset.x, dot_spacing);
             x < canvas_size.x; x += dot_spacing) {
...
void DrawPinShape(const ImVec2 &pin_pos, const ImPinData &pin,
    ...
    case ImNodesPinShape_Circle: {
        GImNodes->CanvasDrawList->AddCircle(
            pin_pos, GImNodes->Style.PinCircleRadius, pin_color,
            CIRCLE_NUM_SEGMENTS, GImNodes->Style.PinLineThickness);
    } break;
    ...
    case ImNodesPinShape_Quad: {
        const QuadOffsets offset =
            CalculateQuadOffsets(GImNodes->Style.PinQuadSideLength);
        GImNodes->CanvasDrawList->AddQuad(
            pin_pos + offset.TopLeft, pin_pos + offset.BottomLeft,
            pin_pos + offset.BottomRight, pin_pos + offset.TopRight, pin_color,
    ...
    case ImNodesPinShape_Triangle: {
        ...
        GImNodes->CanvasDrawList->AddTriangle(
            pin_pos + offset.TopLeft, pin_pos + offset.BottomLeft,
            pin_pos + offset.Right, pin_color,
...
void DrawPin(ImNodesEditorContext &editor, const int pin_idx) {
    ...
    pin.Pos = GetScreenSpacePinCoordinates(parent_node_rect, pin.AttributeRect,
                                           pin.Type);
...
void DrawNode(ImNodesEditorContext &editor, const int node_idx) {
    ...
    const bool node_hovered = GImNodes->HoveredNodeIdx == node_idx &&
                              editor.ClickInteraction.Type !=
...

    {
        ...
        GImNodes->CanvasDrawList->AddRectFilled(
            node.Rect.Min, node.Rect.Max, node_background,
        ...
        if (node.TitleBarContentRect.GetHeight() > 0.f) {
            ...
            GImNodes->CanvasDrawList->AddRectFilled(
                ...
                node.LayoutStyle.CornerRounding, ImDrawCornerFlags_Top);
#else
            GImNodes->CanvasDrawList->AddRectFilled(
                title_bar_rect.Min, title_bar_rect.Max, titlebar_background,
                node.LayoutStyle.CornerRounding, ImDrawFlags_RoundCornersTop);
        ...
        if ((GImNodes->Style.Flags & ImNodesStyleFlags_NodeOutline) != 0) {
            ...
            GImNodes->CanvasDrawList->AddRect(
                node.Rect.Min, node.Rect.Max, node.ColorStyle.Outline,
                node.LayoutStyle.CornerRounding, ImDrawCornerFlags_All,
...
#endif
        cubic_bezier.P0, cubic_bezier.P1, cubic_bezier.P2, cubic_bezier.P3,
        link_color, GImNodes->Style.LinkThickness, cubic_bezier.NumSegments);
}

void BeginPinAttribute(const int id, const ImNodesAttributeType type,
                       const ImNodesPinShape shape, const int node_idx) {
...
static inline bool IsMiniMapHovered() {
    ...
    return IsMiniMapActive() &&
           ImGui::IsMouseHoveringRect(editor.MiniMapRectScreenSpace.Min,
...
static inline void CalcMiniMapLayout() {
    ...
    {
        const ImVec2 max_size = ImFloor(
            editor_rect.GetSize() * editor.MiniMapSizeFraction - border * 2.0f);
        const float max_size_aspect_ratio = max_size.x / max_size.y;
        const ImVec2 grid_content_size =
            editor.GridContentBounds.IsInverted()
                ? max_size
                : ImFloor(editor.GridContentBounds.GetSize());
        const float grid_content_aspect_ratio =
            grid_content_size.x / grid_content_size.y;
    ...
    ImVec2 mini_map_pos;
    {
        ...
        const ImVec2 bottom_right_pos =
            editor_rect.Max - offset - border - mini_map_size;
    ...
    editor.MiniMapContentScreenSpace =
        ImRect(mini_map_pos, mini_map_pos + mini_map_size);
...
static void MiniMapDrawNode(ImNodesEditorContext &editor, const int node_idx) {
    ...
    // Round to near whole pixel value for corner-rounding to prevent visual
    // glitches
    ...
    if (editor.ClickInteraction.Type == ImNodesClickInteractionType_None &&
        ...
        mini_map_node_background =
            GImNodes->Style.Colors[ImNodesCol_MiniMapNodeBackground];
    }

    const ImU32 mini_map_node_outline =
        GImNodes->Style.Colors[ImNodesCol_MiniMapNodeOutline];

    GImNodes->CanvasDrawList->AddRectFilled(node_rect.Min, node_rect.Max,
                                            mini_map_node_background,
                                            mini_map_node_rounding);

    GImNodes->CanvasDrawList->AddRect(node_rect.Min, node_rect.Max,
...
static void MiniMapDrawLink(ImNodesEditorContext &editor, const int link_idx) {
    ...
    const ImU32 link_color =
        GImNodes->Style.Colors[editor.SelectedLinkIndices.contains(link_idx)
...
#endif
        cubic_bezier.P0, cubic_bezier.P1, cubic_bezier.P2, cubic_bezier.P3,
        link_color, GImNodes->Style.LinkThickness * editor.MiniMapScaling,
        cubic_bezier.NumSegments);
}
...
static void MiniMapUpdate() {
    ...
    if (IsMiniMapHovered()) {
        mini_map_background =
            GImNodes->Style.Colors[ImNodesCol_MiniMapBackgroundHovered];
    } else {
        mini_map_background =
            GImNodes->Style.Colors[ImNodesCol_MiniMapBackground];
    }

    // Create a child window bellow mini-map, so it blocks all mouse interaction
    // on canvas.
    int flags = ImGuiWindowFlags_NoBackground;
    ImGui::SetCursorScreenPos(editor.MiniMapRectScreenSpace.Min);
    ImGui::BeginChild("minimap", editor.MiniMapRectScreenSpace.GetSize(), false,
                      flags);
    ...
    GImNodes->CanvasDrawList->AddRect(
        mini_map_rect.Min, mini_map_rect.Max,
    ...
    GImNodes->CanvasDrawList->PushClipRect(
        mini_map_rect.Min, mini_map_rect.Max,
        true /* intersect with editor clip-rect */);

    // Draw links first so they appear under nodes, and we can use the same draw
    // channel
    for (int link_idx = 0; link_idx < editor.Links.Pool.size(); ++link_idx) {
        if (editor.Links.InUse[link_idx]) {
    ...
    {
        const ImU32 canvas_color =
            GImNodes->Style.Colors[ImNodesCol_MiniMapCanvas];
        const ImU32 outline_color =
            GImNodes->Style.Colors[ImNodesCol_MiniMapCanvasOutline];
        const ImRect rect =
            ScreenSpaceToMiniMapSpace(editor, GImNodes->CanvasRectScreenSpace);

        GImNodes->CanvasDrawList->AddRectFilled(rect.Min, rect.Max,
                                                canvas_color);
    ...
    bool center_on_click =
        mini_map_is_hovered && ImGui::IsMouseDown(ImGuiMouseButton_Left) &&
        editor.ClickInteraction.Type == ImNodesClickInteractionType_None &&
...
template <typename T>
void SelectObject(const ImObjectPool<T> &objects,
                  ImVector<int> &selected_indices, const int id) {
...
template <typename T>
void ClearObjectSelection(const ImObjectPool<T> &objects,
                          ImVector<int> &selected_indices, const int id) {
    const int idx = ObjectPoolFind(objects, id);
    IM_ASSERT(idx >= 0);
    IM_ASSERT(selected_indices.find(idx) != selected_indices.end());
...
template <typename T>
bool IsObjectSelected(const ImObjectPool<T> &objects,
                      ImVector<int> &selected_indices, const int id) {
    const int idx = ObjectPoolFind(objects, id);
...
ImNodesStyle::ImNodesStyle()
    : GridSpacing(24.f), NodeCornerRounding(4.f), NodePadding(8.f, 8.f),
      NodeBorderThickness(1.f), LinkThickness(3.f),
      LinkLineSegmentsPerLength(0.1f), LinkHoverDistance(10.f),
      PinCircleRadius(4.f), PinQuadSideLength(7.f), PinTriangleSideLength(9.5),
      PinLineThickness(1.f), PinHoverRadius(10.f), PinOffset(0.f),
...
void StyleColorsDark(ImNodesStyle *dest) {
    ...
    dest->Colors[ImNodesCol_MiniMapBackgroundHovered] =
        IM_COL32(25, 25, 25, 200);
    dest->Colors[ImNodesCol_MiniMapOutline] = IM_COL32(150, 150, 150, 100);
    dest->Colors[ImNodesCol_MiniMapOutlineHovered] =
        IM_COL32(150, 150, 150, 200);
    dest->Colors[ImNodesCol_MiniMapNodeBackground] =
    ...
    dest->Colors[ImNodesCol_MiniMapLinkSelected] =
        dest->Colors[ImNodesCol_LinkSelected];
    dest->Colors[ImNodesCol_MiniMapCanvas] = IM_COL32(200, 200, 200, 25);
    dest->Colors[ImNodesCol_MiniMapCanvasOutline] =
...
void StyleColorsLight(ImNodesStyle *dest) {
    ...
    dest->Colors[ImNodesCol_NodeBackgroundHovered] =
        IM_COL32(240, 240, 240, 255);
    dest->Colors[ImNodesCol_NodeBackgroundSelected] =
        IM_COL32(240, 240, 240, 255);
    dest->Colors[ImNodesCol_NodeOutline] = IM_COL32(100, 100, 100, 255);
    ...
    ImGui::BeginGroup();
    {
...

        {
            ...
            GImNodes->CanvasRectScreenSpace =
                ImRect(EditorSpaceToScreenSpace(ImVec2(0.f, 0.f)),
                       EditorSpaceToScreenSpace(canvas_size));
...
void EndNodeEditor() {
    ...
    // Detect ImGui interaction first, because it blocks interaction with the
    // rest of the UI
    ...
    // Detect which UI element is being hovered over. Detection is done in a
    // hierarchical fashion, because a UI element being hovered excludes any
    // other as being hovered over.

    // Don't do hovering detection for nodes/links/pins when interacting with
    // the mini-map, since its an *overlay* with its own interaction behavior
    // and must have precedence during mouse interaction.

    if ((editor.ClickInteraction.Type == ImNodesClickInteractionType_None ||
         editor.ClickInteraction.Type ==
             ImNodesClickInteractionType_LinkCreation) &&
        MouseInCanvas() && !IsMiniMapHovered()) {
        // Pins needs some special care. We need to check the depth stack to see
        // which pins are being occluded by other nodes.
        ResolveOccludedPins(editor, GImNodes->OccludedPinIndices);

        GImNodes->HoveredPinIdx =
            ResolveHoveredPin(editor.Pins, GImNodes->OccludedPinIndices);

        if (!GImNodes->HoveredPinIdx.HasValue()) {
            // Resolve which node is actually on top and being hovered using the
            // depth stack.
            GImNodes->HoveredNodeIdx =
                ResolveHoveredNode(editor.NodeDepthOrder);
        }

        // We don't check for hovered pins here, because if we want to detach a
        // link by clicking and dragging, we need to have both a link and pin
        // hovered.
        if (!GImNodes->HoveredNodeIdx.HasValue()) {
            GImNodes->HoveredLinkIdx =
                ResolveHoveredLink(editor.Links, editor.Pins);
        }
    }
    ...
    // In order to render the links underneath the nodes, we want to first
    // select the bottom draw channel.
    ...
    // Render the click interaction UI elements (partial links, box selector) on
    ...
    if (!IsMiniMapHovered()) {
        ...
        if (should_auto_pan && !MouseInCanvas()) {
            ...
            editor.AutoPanningDelta = direction * ImGui::GetIO().DeltaTime *
                                      GImNodes->Io.AutoPanningSpeed;
            editor.Panning += editor.AutoPanningDelta;
        }
    }
    ClickInteractionUpdate(editor);

    // At this point, draw commands have been issued for all nodes (and pins).
    // Update the node pool to detect unused node slots and remove those indices
    // from the depth stack before sorting the node draw commands by depth.
...
void MiniMap(const float minimap_size_fraction,
    ...
    // of of the state for the mini map in GImNodes for the actual
    // drawing/updating
}
...
void BeginNode(const int node_id) {
    ...
    ImNodeData &node = editor.Nodes.Pool[node_idx];
    node.ColorStyle.Background =
        GImNodes->Style.Colors[ImNodesCol_NodeBackground];
    node.ColorStyle.BackgroundHovered =
        GImNodes->Style.Colors[ImNodesCol_NodeBackgroundHovered];
    node.ColorStyle.BackgroundSelected =
        GImNodes->Style.Colors[ImNodesCol_NodeBackgroundSelected];
    ...
    node.ColorStyle.TitlebarHovered =
        GImNodes->Style.Colors[ImNodesCol_TitleBarHovered];
    node.ColorStyle.TitlebarSelected =
        GImNodes->Style.Colors[ImNodesCol_TitleBarSelected];
    ...
    ImGui::SetCursorPos(
        GridSpaceToEditorSpace(editor, GetNodeTitleBarOrigin(node)));
...
void EndNode() {
    ...
    if (node.Rect.Contains(GImNodes->MousePos)) {
        GImNodes->NodeIndicesOverlappingWithMouse.push_back(
            GImNodes->CurrentNodeIdx);
    }
}
...
void EndNodeTitleBar() {
    ...
    ImGui::SetCursorPos(
        GridSpaceToEditorSpace(editor, GetNodeContentOrigin(node)));
}

void BeginInputAttribute(const int id, const ImNodesPinShape shape) {
    BeginPinAttribute(id, ImNodesAttributeType_Input, shape,
                      GImNodes->CurrentNodeIdx);
}
...
void BeginOutputAttribute(const int id, const ImNodesPinShape shape) {
    BeginPinAttribute(id, ImNodesAttributeType_Output, shape,
                      GImNodes->CurrentNodeIdx);
}
...
void Link(const int id, const int start_attr_id, const int end_attr_id) {
    ...
    if ((editor.ClickInteraction.Type ==
             ImNodesClickInteractionType_LinkCreation &&
...
void PushColorStyle(const ImNodesCol item, unsigned int color) {
    GImNodes->ColorModifierStack.push_back(
        ImNodesColElement(GImNodes->Style.Colors[item], item));
...
static const ImNodesStyleVarInfo GStyleVarInfo[] = {
    ...
    {ImGuiDataType_Float, 1,
     (ImU32)offsetof(ImNodesStyle, NodeBorderThickness)},
    // ImNodesStyleVar_LinkThickness
    {ImGuiDataType_Float, 1, (ImU32)offsetof(ImNodesStyle, LinkThickness)},
    // ImNodesStyleVar_LinkLineSegmentsPerLength
    {ImGuiDataType_Float, 1,
     (ImU32)offsetof(ImNodesStyle, LinkLineSegmentsPerLength)},
    ...
    {ImGuiDataType_Float, 1,
     (ImU32)offsetof(ImNodesStyle, PinTriangleSideLength)},
...
void PopStyleVar(int count) {
    while (count > 0) {
        ...
        const ImNodesStyleVarElement style_backup =
            GImNodes->StyleModifierStack.back();
        GImNodes->StyleModifierStack.pop_back();
        const ImNodesStyleVarInfo *var_info =
            GetStyleVarInfo(style_backup.Item);
        void *style_var = var_info->GetVarPtr(&GImNodes->Style);
        if (var_info->Type == ImGuiDataType_Float && var_info->Count == 1) {
            ((float *)style_var)[0] = style_backup.FloatValue[0];
        } else if (var_info->Type == ImGuiDataType_Float &&
                   var_info->Count == 2) {
            ((float *)style_var)[0] = style_backup.FloatValue[0];
...
bool IsAnyAttributeActive(int *const attribute_id) {
    IM_ASSERT((GImNodes->CurrentScope &
               (ImNodesScope_Node | ImNodesScope_Attribute)) == 0);
...
bool IsLinkStarted(int *const started_at_id) {
    ...
    const bool is_started =
        (GImNodes->ImNodesUIState & ImNodesUIState_LinkStarted) != 0;
    if (is_started) {
        const ImNodesEditorContext &editor = EditorContextGet();
...
bool IsLinkDropped(int *const started_at_id,
                   const bool including_detached_links) {
    // Call this function after EndNodeEditor()!
    IM_ASSERT(GImNodes->CurrentScope == ImNodesScope_None);
...
bool IsLinkCreated(int *const started_at_pin_id, int *const ended_at_pin_id,
    ...
    const bool is_created =
        (GImNodes->ImNodesUIState & ImNodesUIState_LinkCreated) != 0;
    ...
    if (is_created) {
        ...
        if (created_from_snap) {
            *created_from_snap = editor.ClickInteraction.Type ==
                                 ImNodesClickInteractionType_LinkCreation;
        }
    }
...
bool IsLinkCreated(int *started_at_node_id, int *started_at_pin_id,
                   int *ended_at_node_id, int *ended_at_pin_id,
    ...
    if (is_created) {
        ...
        const int end_idx =
            editor.ClickInteraction.LinkCreation.EndPinIdx.Value();
        const ImPinData &start_pin = editor.Pins.Pool[start_idx];
        const ImNodeData &start_node =
            editor.Nodes.Pool[start_pin.ParentNodeIdx];
        const ImPinData &end_pin = editor.Pins.Pool[end_idx];
...
const char *
SaveEditorStateToIniString(const ImNodesEditorContext *const editor_ptr,
...
void LoadCurrentEditorStateFromIniString(const char *const data,
                                         const size_t data_size) {
    LoadEditorStateFromIniString(&EditorContextGet(), data, data_size);
}

void LoadEditorStateFromIniString(ImNodesEditorContext *const editor_ptr,
    ...
    ImNodesEditorContext &editor =
        editor_ptr == NULL ? EditorContextGet() : *editor_ptr;
...
void SaveEditorStateToIniFile(const ImNodesEditorContext *const editor,
...
void LoadEditorStateFromIniFile(ImNodesEditorContext *const editor,
                                const char *const file_name) {
    size_t data_size = 0u;
